...
G_DEFINE_FINAL_TYPE (FramePlatform, frame_platform, GTK_TYPE_FRAME)
...
static void
set_platform (FramePlatform *self,
              guint32                        id)
{
  switch (id)
    {
    case PLATFORM_NES:
      self->box_palette = g_object_new (NES_TYPE_PALETTE,
                                        "orientation",
...
frame_palette_set_property (GObject      *object,
                     ...
                     GParamSpec   *spec)
{
  FramePlatform *self = FRAME_PLATFORM (object);
...
static void
frame_platform_class_init (FramePlatformClass *klass)
{
...
static void
frame_platform_init (FramePlatform *self)
{
